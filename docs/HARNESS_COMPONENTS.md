...
| 9 | Permissions | Partial | `AGENTS.md`, `docs/HARNESS.md`, `docs/FEATURE_INTAKE.md`, `docs/ARCHITECTURE.md`, `agent/src/security/permissionPolicy.ts`, `agent/src/tools/gateway.ts`, `agent/src/security/approvalService.ts`, installer conflict handling in `scripts/install-harness.sh` | Harness instructions govern agent changes; the local operator uses `ToolGateway` before execution, contextual default allow, scoped approvals, and destructive denial. It is application-level safety, not an OS sandbox. | P1 authority model complete under ADR 0017 (source-managed custom-tool registry, extended approval matching, persisted-approval restart). P2 audit work complete: gateway audit records (P2.1) and side-effect bypass audit (P2.2 — boundary locked by `test/gateway-boundary.test.js`). P2.3 operational proof complete: per-session concurrency serialization, cancellation through approval resume (AbortSignal forwarded on resume; `/stop` cancels paused approvals), run-deadline abort, artifact/raw-AI retention, and graceful shutdown extracted to `core/shutdown.ts`. P2.1–P2.6 complete: gateway audit records (P2.1), side-effect bypass audit (P2.2), operational proof (P2.3), scheduler reliability (P2.4), CDP host guardrails (P2.5), and allowlist→shortcut terminology rename (P2.6). P2.7 final migration proof complete (SQLite migration from an existing old-schema DB, CLI both-halves + Telegram callback_query approval smoke, scheduler prepareEffect; TypeScript/regression, destructive refusal, and custom-tool execution already covered). ADR 0017 P2 cutover fully complete. |
