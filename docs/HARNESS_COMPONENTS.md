...
| 9 | Permissions | Partial | `AGENTS.md`, `docs/HARNESS.md`, `docs/FEATURE_INTAKE.md`, `docs/ARCHITECTURE.md`, `agent/src/security/permissionPolicy.ts`, `agent/src/tools/gateway.ts`, `agent/src/security/approvalService.ts`, installer conflict handling in `scripts/install-harness.sh` | Harness instructions govern agent changes; the local operator uses `ToolGateway` before execution, contextual default allow, scoped approvals, and destructive denial. It is application-level safety, not an OS sandbox. | Complete source-managed custom-tool registry integration and reconcile legacy story evidence with ADR 0017. |
