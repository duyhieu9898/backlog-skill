...
| 9 | Permissions | Partial | `AGENTS.md`, `docs/HARNESS.md`, `docs/FEATURE_INTAKE.md`, `docs/ARCHITECTURE.md`, `agent/src/security/permissionPolicy.ts`, `agent/src/tools/files.ts`, `agent/commands.json`, installer conflict handling in `scripts/install-harness.sh` | Harness instructions govern agent changes; the local operator enforces typed workspace, path-denial, command-cwd, file-tool, and confirmation decisions before local effects. | Exact-action confirmation and validated AI tool routing remain pending; OS-level sandboxing is deferred. |
