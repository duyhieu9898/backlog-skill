...
export function validateJsonSchema(schema: JsonSchema, value: unknown, path = "input"): string[] {
  if (schema.type === "object") {
    ...
    for (const [key, child] of Object.entries(schema.properties)) {
      if (key in object) errors.push(...validateJsonSchema(child, object[key], `${path}.${key}`));
    }
    return errors;
  }
  ...
  if (schema.type === "string") {
    ...
    if (schema.format === "date" && !isCanonicalDate(value)) errors.push(`${path} must be YYYY-MM-DD.`);
