import type { ActiveScopeLease, Capability, CapabilityRoute } from "../brain/provider";
...
export function generalRoute(reason: string, continuation: CapabilityRoute["continuation"] = "new"): CapabilityRoute {
  return { capabilities: [], targets: [], continuation, confidence: "low", selectionReason: reason };
}

export function resolveCapabilityRoute(input: { text: string; traceId: string; now?: Date; activeLease?: ActiveScopeLease | null; skillSlug?: string }): { route: CapabilityRoute; lease: ActiveScopeLease | null } {
  const text = input.text.toLowerCase();
  const now = input.now || new Date();
  const expired = input.activeLease && new Date(input.activeLease.expiresAt).getTime() <= now.getTime();
  if (has(text, CANCEL)) return { route: generalRoute("explicit cancellation", "cleared"), lease: null };
  if (has(text, GENERAL) && !has(text, [...FILE_READ, ...WEB, ...DESKTOP, ...COMMAND])) return { route: generalRoute("self-contained general question", "cleared"), lease: null };
  let capabilities: Capability[] = [];
  let reason = "unresolved request";
  let confidence: CapabilityRoute["confidence"] = "low";
  let skillSlug = input.skillSlug;
  if (skillSlug) { capabilities = ["skill"]; reason = "selected skill"; confidence = "hard-signal"; }
  else if (has(text, FILE_WRITE)) { capabilities = ["file-write", "file-read"]; reason = "explicit file write"; confidence = "hard-signal"; }
  else if (has(text, FILE_READ)) { capabilities = ["file-read"]; reason = "explicit file read"; confidence = "hard-signal"; }
