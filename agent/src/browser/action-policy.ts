...
export type BrowserActionPolicyContext = {
  sessionId: string;
  runId: string;
  profile: string;
  targetId: string;
...
export function computeActionFingerprint(context: Omit<BrowserActionPolicyContext, "runId" | "sessionId">): string {
  ...
  if (context.action.kind === "fill") {
    const isSensitive =
      context.element?.inputType === "password" ||
...
export function evaluateAction(
  ...
  config: BrowserPermissionConfig,
): PolicyDecision {
  ...
  let risk: BrowserActionRisk = isSensitive ? "sensitive-input" : "none";
        ...
        if (!isHarmless && role !== "link") {
          ...
          if (!isSearch) {
            ...
            if (DESTRUCTIVE_TERMS.test(textToMatch)) {
              ...
              CONSEQUENTIAL_TERMS.test(textToMatch) ||
              role === "textarea" ||
