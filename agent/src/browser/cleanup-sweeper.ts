...
export type CleanupSweepResult = {
  ...
  closedIdleTabs: string[];
  closedQuotaTabs: string[];
  skippedBusyTabs: string[];

  deletedSnapshots: string[];
  errors: Array<{
    resourceType: "tab" | "snapshot" | "profile";
    resourceId: string;
    code: string;
    message: string;
  }>;
...
export class BrowserCleanupSweeper {
  ...
  async sweep(): Promise<CleanupSweepResult> {
    ...
    log.info(this.traceId, "browser.cleanup.started", { startedAt });

    const closedIdleTabs: string[] = [];
    const closedQuotaTabs: string[] = [];
    const skippedBusyTabs: string[] = [];
    const errors: CleanupSweepResult["errors"] = [];
    ...
    try {
      ...
      for (const p of activeProfiles) {
        ...
        const idleEligible = tabs.filter(t => {
          ...
          if (isStale && isBusy) {
            ...
            log.info(this.traceId, "browser.cleanup.tab_skipped_busy", { profileName: p.name, targetId: t.targetId, lastUsedAt: t.lastUsedAt });
          }
        ...
        if (idleEligible.length > 0) {
          ...
          for (const tab of idleEligible) {
            ...
            try {
              ...
              log.info(this.traceId, "browser.cleanup.tab_closed", { profileName: p.name, targetId: tab.targetId, reason: "idle" });
            } catch (err: any) {
        ...
        if (remainingTabs.length > maxTabs) {
          ...
          if (quotaEligible.length === 0) {
            ...
            log.warn(this.traceId, "browser.cleanup.failed", { profileName: p.name, code: "TAB_QUOTA_DEFERRED" });
            continue;
          }
          ...
          for (const tab of toClose) {
            try {
              ...
              log.info(this.traceId, "browser.cleanup.tab_closed", { profileName: p.name, targetId: tab.targetId, reason: "quota" });
            } catch (err: any) {
              errors.push({ resourceType: "tab", resourceId: tab.targetId, code: err.code || "CLOSE_FAILED", message: err.message });
            }
          }
      ...
      const deletedSnapshots = refStore.pruneExpired();
      for (const snapId of deletedSnapshots) {
        log.info(this.traceId, "browser.cleanup.snapshot_deleted", { snapshotId: snapId });
      }

      const completedAt = Date.now();
      log.info(this.traceId, "browser.cleanup.completed", {
      ...
      log.error(this.traceId, "browser.cleanup.failed", { error: e });
      throw e;
    } finally {
